...
namespace FundaWebApplication.Controllers
{
    ...
    public class FundaController : ControllerBase
    {
        ...
        [HttpGet("top10withtuin")]
        public async Task<ActionResult<IEnumerable<ResultByAgencyModel>>> GetTop10WithTuin()
        {
            try
            {
                // Retrieve a list of agencies with their properties that contain a yard
                // Node: These agencies already come ordered by number of properties desc
                var agencies = await _fundaService.GetAgenciesWithMostPropertiesForSale("/amsterdam/tuin");
                // Retrieve the top 10
                var top10Agencies = agencies.Take(10).ToList();
                // Return success
                return Ok(top10Agencies);
